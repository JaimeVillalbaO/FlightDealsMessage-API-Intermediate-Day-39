...
sheet_endpoint = 'https://api.sheety.co/ef48affbf9bd70ec9ed14b75b25b59c8/flightsDeals/prices'
...
today = datetime.now()
range_dates = today + timedelta(days=30)
...
for i in range(len(code_citys)):
    ...
    try:
        ...
        price_updated.append(response.json()['data'][0]['conversion']['GBP'])
        ...
        if cero_step == 'yes':
            ...
            with smtplib.SMTP('smtp.gmail.com', 587) as connection:
                ...
                connection.sendmail(from_addr=my_email, 
                                    ...
                                    msg=(f'Lowest Price Alert! 🚨 Only {price_updated[p]} £ to fly from London-{airport_from[p]} to '
                                        f'{citys[p]}-{airport_to[p]} from {date_from[p]} to {date_to[p]}.').encode('utf-8'),
                                    )
            ...
            with smtplib.SMTP('smtp.gmail.com', 587) as connection:
                ...
                connection.sendmail(from_addr=my_email, 
                                    ...
                                    msg=(f'Lowest Price Alert! 🚨 Only {price_updated[p]} £ to fly from London-{airport_from[p]} to '
                                        f'{citys[p]}-{airport_to[p]} from {date_from[p]} to {date_to[p]}. Flight has 1 step over, \
                                            via London-{city_step_from[p]}-{citys[p]} and {citys[p]}-{city_step_to[p]}-London.').encode('utf-8'),
                                    )
